...
| `--target <n>` | Total de PRs mergeadas a alcanzar; crea sólo las que falten |
...
Cada pull request son 5 peticiones (rama, commit, PR, merge y leer el head). GitHub aplica dos límites: unas **80 peticiones que crean contenido por minuto** y **5000 peticiones por hora**.

Una tirada hasta oro son unas 5100 peticiones, así que *supera* la cuota horaria sí o sí. El cliente lo gestiona: espacia las escrituras, pausa hasta el reset de cuota, respeta el `Retry-After` y guarda un checkpoint cada 10 PRs para poder retomar una ejecución cortada.
...
| bronce | 16 | ~1,5 min |
| plata | 128 | ~11 min |
| oro | 1024 | ~85 min + una pausa de cuota |
...
src/state.js            historial local y checkpoints de tiradas largas
src/log.js              salida en consola
src/commands/           un archivo por comando
test/                   suite con node:test, sin framework externo
